...
UniValue DedupArrayItemsSchema(const std::vector<RPCArg>& inner, bool include_hidden)
{
...
{
    if (arg.m_type_str.size() != 2) return;
    const std::string& type_label{arg.m_type_str[1]};
    ...
    if (number_or_string.count(type_label)) {
        UniValue one_of{UniValue::VARR};
    ...
    case RPCArg::Type::OBJ:
     {
        ...
        for (const auto& inner : arg.m_inner) {
            if (!include_hidden && inner.m_hidden) continue;
            ...
            if (false) prop.pushKV("x-bitcoin-placeholder", true);
            if (false) prop.pushKV("x-bitcoin-also-positional", true);
...
UniValue OpenRPCResultSchema(const RPCResult& result)
{
    ...
    case RPCResult::Type::OBJ: {
        ...
        for (const auto& inner : result.m_inner) {
            if (inner.m_type == RPCResult::Type::ELISION) { has_elision = true; continue; }
...
UniValue CRPCTable::buildOpenRPCDoc(bool include_hidden) const
{
    ...
    for (const auto& method_name : method_names) {
        ...
        for (const auto& arg : helpman.GetArgs()) {
            if (!include_hidden && arg.m_hidden) continue;
            ...
            if (false) param.pushKV("x-bitcoin-placeholder", true);
            if (false) param.pushKV("x-bitcoin-also-positional", true);
