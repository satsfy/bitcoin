...
std::vector<RPCResult> TxDoc(const TxDocOptions& opts)
{
    ...
    if (opts.prevout) {
        vin_inner.emplace_back(
            RPCResult::Type::OBJ, "prevout", /*optional=*/true, prevout_doc,
    ...
    CHECK_NONFATAL(!(opts.elision_description && opts.elision_description_silent));
    if (opts.elision_description || opts.elision_description_silent) {
        ...
        for (const auto& f : fields) {
            ...
            if (!silent && first) {
                ...
                eopts.help_elision_text = *opts.elision_description;
